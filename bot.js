...
client.on("message", async msg => {
  if (msg.author.bot) undefined;
  if (!msg.content.startsWith(process.env.PREFIX)) undefined;
  ...
  if (command === "play") {
    ...
    if (!voiceChannel)
      msg.channel.send(
        "Je suis désolé mais tu dois être dans un channel vocal !"
      );
...
Veuillez entrer un numéro correspondant à la musique de votre choix.
    ...
    serverQueue.connection.dispatcher.end(
      "Tu viens de passer à la musique suivante !"
    );
    undefined;
...
**Now playing:** ${serverQueue.songs[0].title}
    ...
    if (serverQueue && serverQueue.playing) {
      ...
      msg.channel.send("⏸ La musique est en pause !");
    }
    ...
    if (serverQueue && !serverQueue.playing) {
      ...
      msg.channel.send("▶ La musique est rejouée !");
    }
    msg.channel.send("Il n'y a rien dans la playlist.");
  }
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
  ...
  if (!serverQueue) {
    ...
    else msg.channel.send(`✅ **${song.title}** a été ajouté!`);
  }
  undefined;
}
